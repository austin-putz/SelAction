...
960         print *,"common environmental effect in the progeny test"
  ...
  !            print *," "
          ...
          do p=1,2*nclass
            if (pvalcl(p).gt.0.0) then
              if (p.le.nclass) then
                tempresponse=(oi(p)*sqrt(osigmai(p))*nselec(p))/(genint*sselec)
          !      print *,"oi(p)",oi(p)
            ...
            !    print *,"pvalcl(p)",pvalcl(p)
             !   print *,"genints",genints
              !  print *,"sumpvals",sumpvals
                ...
                tempresponse=(oi(p)*sqrt(osigmai(p))*nselec(p))/(genint*dselec)
      !          print *,"oi(p)",oi(p)
    !            print *,"osigmai",p,osigmai(p)
        !        print *,"pvalcl(p)",pvalcl(p)
              ...
              if (pvalcl(q).gt.0.0) then
                ...
                if (pvalcl(q).gt.0.0) then
                  ...
                  if (q.le.nclass) then
                    ...
                    tempresponse=(tempresponse*nselec(q))/(genint*sselec)
                    sresponse(p)=sresponse(p)+tempresponse
                  else
                    tempresponse4=(matmul(transpose(sb),spartrealg)*oi(q))/(sqrt(osigmai(q)))
                    tempresponse=tempresponse4(1,1)
                    tempresponse=(tempresponse*nselec(q))/(genint*dselec)
	...
	! print output
	  ...
	  write(unit=20, fmt=*) "   ",xtraits(p)
 	  ...
 	  if (desttraits(p).eq."h" .or. desttraits(p).eq."b") then
        ...
        write(unit=20, fmt=*) " "
        write(unit=20, fmt='(a23,f5.2,a18)') " generation interval : ",(genints+genintd)/2," cohort intervals"
	write(unit=20, fmt=*) " "
